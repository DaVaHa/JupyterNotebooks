...
files = [f for f in os.listdir(path) if f.endswith('.csv') and 'winemag' in f and ('2015' in f or '2016' in f)]
#print(files)
...
for f in front_files:
    ...
    if f == 'winemag_v2018.csv':
        df = pd.read_csv(path + f, delimiter='|', header=None, skiprows=[16871])
    elif f == 'winemag_v1000_2000.csv':
        df = pd.read_csv(path + f, delimiter='|', header=None, skiprows=[5044])
...
full.to_csv(path + "dataset_15_16.csv")
